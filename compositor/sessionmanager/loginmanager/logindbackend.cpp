...
LogindBackend *LogindBackend::create(SessionManager *sm, const QDBusConnection &connection)
{
    ...
    backend->m_interface->connection().connect(login1Service, login1Object, login1ManagerInterface,
                                               QStringLiteral("PrepareForSleep"), backend,
                                               SLOT(prepareForSleep(bool)));

    // Get notified when the system is preparing for shutdown
    backend->m_interface->connection().connect(login1Service, login1Object, login1ManagerInterface,
                                               QStringLiteral("PrepareForShutdown"), backend,
...
int LogindBackend::takeDevice(const QString &path)
{
    ...
    if (reply.type() == QDBusMessage::ErrorMessage) {
        qCWarning(LOGIND_BACKEND, "Couldn't take device \"%s\": %s", qPrintable(path),
...
void LogindBackend::switchToVt(int index)
{
    ...
    connect(w, &QDBusPendingCallWatcher::finished, this, [this,
        ...
        Q_FOREACH (const LogindSessionInfo &sessionInfo, reply.value()) {
            QDBusMessage msg =
                QDBusMessage::createMethodCall(login1Service, sessionInfo.sessionPath.path(),
                                               dbusPropertiesInterface, QStringLiteral("GetAll"));
...
void LogindBackend::setupPowerButton()
{
    ...
    connect(w, &QDBusPendingCallWatcher::finished, [this](QDBusPendingCallWatcher *watcher) {
        ...
        if (!reply.isValid()) {
            qCDebug(LOGIND_BACKEND) << "Failed to get power button inhibitors";
            return;
        }

        m_powerButtonFd = dup(reply.value().fileDescriptor());
    });
}
...
void LogindBackend::getSession(QDBusPendingCallWatcher *watcher)
{
    ...
    m_interface->connection().connect(login1Service, m_sessionPath, login1SessionInterface,
                                      QStringLiteral("Lock"), this, SIGNAL(sessionLocked()));
    m_interface->connection().connect(login1Service, m_sessionPath, login1SessionInterface,
                                      QStringLiteral("Unlock"), this, SIGNAL(sessionUnlocked()));
